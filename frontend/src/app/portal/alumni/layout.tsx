...
export default function AlumniPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && me && me.user.user_type !== "ALUMNI" && !me.is_super_admin) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="flex items-center justify-between border-b bg-white px-6 py-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-saffron-700">Alumni Portal</p>
          <nav className="mt-2 flex gap-4 text-sm">
            <Link href="/portal/alumni" className="text-saffron-700 hover:underline">Directory</Link>
