...
export default function ParentPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && me && me.user.user_type !== "PARENT" && !me.is_super_admin) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="border-b bg-white px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-saffron-700">Parent Portal</p>
          <p className="text-sm text-stone-600">{me.user.profile?.display_name || me.user.email}</p>
          <nav className="mt-2 flex gap-4 text-sm">
            <Link href="/portal/parent" className="text-saffron-700 hover:underline">Dashboard</Link>
