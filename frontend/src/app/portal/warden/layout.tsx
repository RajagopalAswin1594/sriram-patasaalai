...
export default function WardenPortalLayout({ children }: { children: React.ReactNode }) {
  const { me, loading, logout } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (!loading && !me) router.replace("/login");
    if (!loading && me && me.user.user_type !== "HOSTEL_WARDEN" && !me.is_super_admin) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="border-b bg-white px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-saffron-700">Warden Portal</p>
          <p className="text-sm text-stone-600">{me.user.profile?.display_name || me.user.email}</p>
          <nav className="mt-2 flex gap-4 text-sm">
            <Link href="/portal/warden" className="text-saffron-700 hover:underline">Dashboard</Link>
